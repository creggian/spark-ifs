...
import creggian.mrmr.feature.common.Score
import creggian.mrmr.feature.common.AbstractScore

class MRMR {

	def columnWise(sc: SparkContext, rdd: org.apache.spark.rdd.RDD[Array[Double]], nfs: Int, classLevels: Array[Double], featuresLevels: Array[Double], classIdx: Int): Array[(Int, Double)] = {
		val classLevels_bc = sc.broadcast(classLevels)
		val featuresLevels_bc = sc.broadcast(featuresLevels)
		val classIdx_bc = sc.broadcast(classIdx)

		val ncols = rdd.take(1)(0).length

		var candidateIdx = (0 until ncols).toArray.diff(Array(classIdx))
		...
		for (i <- 1 to nfs) {
			...
			val step1Class = rdd.mapPartitions(iter => {
				...
				while(iter.hasNext) {
					val arr = iter.next
					val classValue = arr(clIdx)
					val arrCandidate = caIdx.map(arr)
					val arrSelected = seIdx.map(arr)
					...
					for (j <- 0 until arrCandidateWithIndex.size) {
						...
						val (value, idx) = tuple
						val matWithClassMapKey = (idx, clIdx)
			...
			val step2 = step1ClassReduceMap.union(step1FeaturesReduceMap).groupByKey.map(entry => {
				...
				var score = new Score()
				var mrmrClass = score.computeMI(matWithClass)

				var mrmrFeatures = 0.0
				for (f <- matWithFeatures) {
					mrmrFeatures = mrmrFeatures + score.computeMI(f)
				}

				var coefficient = 1.0
				if (seIdx.size > 1) {
					coefficient = 1.0 / (seIdx.size).toDouble
				}
				var mrmr_i = mrmrClass - (coefficient * mrmrFeatures)
			...
			println("    idx: "+step3(0)._1)
			println("    score: "+step3(0)._2)
			println("    selected: "+selectedIdx.mkString(", "))
		}

		selectedIdx.zip(selectedIdxScore)
	}

	def rowWise(sc: SparkContext, rdd: org.apache.spark.rdd.RDD[(String, Array[Double])], nfs: Int, classLevels: Array[Double], featuresLevels: Array[Double], classVector: Array[Double], scoreClassName: String): Array[(String, Double)] = {
		val classLevels_bc = sc.broadcast(classLevels)
		val featuresLevels_bc = sc.broadcast(featuresLevels)
		...
		val score = tb.eval(tb.parse("new "+scoreClassName+"()")).asInstanceOf[AbstractScore]
		...
		for (ii <- 1 to nfs) {
			...
			selectedVariable = selectedVariable ++ Array(newSelectedArr)
                        ...
                        println("  iteration "+ii+"th:")
                        println("    label: "+step3(0)._1)
                        println("    score: "+step3(0)._2)
                        println("    selected: "+selectedVariableName.mkString(", "))
		}
